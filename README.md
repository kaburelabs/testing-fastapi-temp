...
- 🎯 Entry points to execute your program using `python -m <testing_fastapi_temp>` or `$ testing_fastapi_temp` with basic CLI argument parsing.
...
---
# testing_fastapi_temp

[![codecov](https://codecov.io/gh/kaburelabs/testing-fastapi-temp/branch/main/graph/badge.svg?token=testing-fastapi-temp_token_here)](https://codecov.io/gh/kaburelabs/testing-fastapi-temp)
[![CI](https://github.com/kaburelabs/testing-fastapi-temp/actions/workflows/main.yml/badge.svg)](https://github.com/kaburelabs/testing-fastapi-temp/actions/workflows/main.yml)

Awesome testing_fastapi_temp created by kaburelabs

## Install

from source
```bash
git clone https://github.com/kaburelabs/testing-fastapi-temp testing_fastapi_temp
cd testing_fastapi_temp
make install
```
...
```bash
pip install testing_fastapi_temp
```

## Executing

```bash
$ testing_fastapi_temp run --port 8080
```

or

```bash
python -m testing_fastapi_temp run --port 8080
```

or

```bash
$ uvicorn testing_fastapi_temp:app
```

## CLI

```bash
❯ testing_fastapi_temp --help
Usage: testing_fastapi_temp [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ testing_fastapi_temp create-user --help
Usage: testing_fastapi_temp create-user [OPTIONS] USERNAME PASSWORD
...
```bash
testing_fastapi_temp create-user admin admin --superuser
```
...
Run with `testing_fastapi_temp run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `testing_fastapi_temp create-user`.
...
-----------------------------------------------------
testing_fastapi_temp/__init__.py              4      0   100%
testing_fastapi_temp/app.py                  16      1    94%
testing_fastapi_temp/cli.py                  21      0   100%
testing_fastapi_temp/config.py                5      0   100%
testing_fastapi_temp/db.py                   10      0   100%
testing_fastapi_temp/models/__init__.py       0      0   100%
testing_fastapi_temp/models/content.py       47      1    98%
testing_fastapi_temp/routes/__init__.py      11      0   100%
testing_fastapi_temp/routes/content.py       52     25    52%
testing_fastapi_temp/routes/security.py      15      1    93%
testing_fastapi_temp/routes/user.py          52     26    50%
testing_fastapi_temp/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in testing_fastapi_temp/default.toml.
